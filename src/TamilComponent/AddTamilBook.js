...
function AddTamilBook() {
    ...
    const { values, handleChange, handleSubmit, handleBlur, errors, touched } = useFormik({
        ...
        onSubmit: (newBook) => {
            // console.log(newBook);
            AddNewTamilBooks(newBook)
        }

    });

    const AddNewTamilBooks = (newBook) => {

        setTamilBook([...tamilbook, newBook]);
        navigate('/tamilbook');
        alert("New Tamil Book added successfully")
    }

  return (
    <BaseApp
    title=" Add A New Tamil Books">
